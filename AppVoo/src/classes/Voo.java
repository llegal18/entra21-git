    ...
    public Voo(int linhas, int cadeirasPorLinha) {
        ...
        for (int i = 0; i < linhas; i++) {
            for (int j = 0; j < cadeirasPorLinha; j++) {
                this.codificacaoAssentos[i][j] = String.valueOf(alfabeto.charAt(i) + (j + 1));
            }
        }
    }

    public boolean verificaoOcupacao(String codigo) {
